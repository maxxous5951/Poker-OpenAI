...
class PokerAssistant:
    ...
    def __init__( self, openai_client, hero_info, game_state, audio_player ):
    ...
    def AnalyzeAI( self, hero_buttons_active, poker_game_data ):

        print(F"{Fore.YELLOW}AnalyzeAI(): Starting AI analysis...")

        gpt4_output = self.analyze_game_state_with_gpt4( hero_buttons_active, poker_game_data )
        ...
        if gpt4_output is not None:
            ...
            return self.extract_hero_action_details_from_gpt4_output( gpt4_output )
    
        else:
            print(F"{Fore.RED}Failed to get response in 26 seconds, so I will FOLD the hand now..")
            #self.execute_check_or_fold(hero_buttons_map)  ==>  Execute check/fold
    ...
    def create_user_prompt( self, realtime_game_data):
        ...
        for player_number in range(1, 7):  # range(1, 7) generates numbers from 1 to 6
            ...
            player_info = self.game_state.players.get(player_number, {})
    ...
    def analyze_game_state_with_gpt4(self, hero_buttons_active, realtime_game_data):
        try:
           ...
           # Format the hero_buttons_active values to include only actions
            available_actions = "\n".join(hero_buttons_active["actions"])
    ...
    def extract_hero_action_details_from_gpt4_output(self, gpt4_output):
        """
            ...
            if action_match:
                ...
                for button_info in hero_buttons_map.values():
                    ...
                    if button_info['action'][:4].lower() in extracted_action.lower():
                        ...
                        break
                '''
                if extracted_action == "Raise" or "Bet":
                   
                    #self.hero_action.execute_action(button_coordinates, extracted_action, action_amount_match)  ==> Execute Raise

                    self.audio_player.play_speech(f"Hero ACTION: {extracted_action} {action_amount_match} dollars. Go!")
                else:
                    #self.hero_action.execute_action(None, "Fold", None)  ==> Execute Fold
